...
export async function createBounty(draft: RequestDraft, token: TokenRecord): Promise<MarketplaceOrder> {
  ...
  const scopeSource = { scope: resolvedWorkType(draft), category: resolvedCategory(draft), project: draft.project.trim(), buyer: draft.buyer.trim(), contactMethod: draft.providerPreference.trim(), deliveryDeadline: canonicalMilestones.at(-1)!.deliveryDeadline, support: parseSupport(draft.support), criteria: parseCriteria(draft.criteria).map(c => c.label), fundOnApplicantAcceptance: true };
