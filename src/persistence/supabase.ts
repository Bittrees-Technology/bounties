...
export type BountyRow = { id: string; creator_id: string; creator_wallet_address?: string; title: string; description: string; scope_source: Record<string, unknown>; scope_hash: `0x${string}`; chain_id: number; token_id: string; token_decimals: number; budget_base_units: string; status: string; escrow_schedule_status?: "structured" | "requires_recreation"; moderation_status?: "visible" | "hidden"; moderation_reason?: string | null; created_at: string; accepted_proposal_id?: string; token: TokenRecord; milestones: ApiMilestone[]; proposals: ApiProposal[]; escrow?: (EscrowObservation & { cancellation_message?: string | null; cancellation_message_hash?: `0x${string}` | null; cancellation_transaction_hash?: `0x${string}` | null }) | null; reviews?: ParticipantReview[] };
