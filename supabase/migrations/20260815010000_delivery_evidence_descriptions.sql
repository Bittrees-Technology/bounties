-- Optional delivery descriptions are immutable offchain supporting context.
-- They are deliberately excluded from bounty-evidence-commitment.v1: the
-- deployed escrow continues to commit the delivered bytes and URI without a
-- contract or commitment-version change.

alter table public.delivery_evidence
  add column description text,
  add constraint delivery_evidence_description_plaintext check (
...
comment on column public.delivery_evidence.description is
  'Optional immutable plain-text delivery context. Offchain only; excluded from bounty-evidence-commitment.v1.';

create function public.app_reject_delivery_evidence_description_change()
returns trigger
language plpgsql set search_path = public as $$
begin
  if new.description is distinct from old.description then
    raise exception 'DELIVERY_EVIDENCE_DESCRIPTION_IMMUTABLE' using errcode = '23514';
  end if;
  return new;
end $$;

create trigger delivery_evidence_description_immutable
before update of description on public.delivery_evidence
for each row execute function public.app_reject_delivery_evidence_description_change();

revoke all on function public.app_reject_delivery_evidence_description_change()
...
begin
  ...
  if p_description is not null and (
    ...
    raise exception 'INVALID_EVIDENCE_DESCRIPTION' using errcode = '22023';
  end if;
  ...
  insert into public.delivery_evidence (
    milestone_id, provider_id, uri, description, content_hash, uri_hash, evidence_salt,
    ...
    p_milestone_id, p_actor_id, p_uri, p_description, lower(p_content_hash), lower(p_uri_hash), lower(p_evidence_salt),
...
revoke execute on function public.app_submit_canonical_delivery_evidence(
...
revoke all on function public.app_submit_canonical_delivery_evidence(
  uuid,uuid,text,text,text,text,text,text,text,text,text,text,integer,bigint,text,text,text,text,text,text
